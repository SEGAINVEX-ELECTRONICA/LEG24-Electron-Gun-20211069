...
uint16_t Adafruit_ADS1015::readADC_SingleEnded(uint8_t channel) {
  ...
  uint16_t config = ADS1015_REG_CONFIG_CQUE_NONE    | // Disable the comparator (default val)
                    ...
                    ADS1015_REG_CONFIG_DR_HIGH    	| // 860 samples per second (1.16ms) (maximum)
                    ADS1015_REG_CONFIG_MODE_SINGLE;    // Continuous conversion mode
...
uint16_t Adafruit_ADS1015::readADC_Contin(uint8_t channel) {
  if (channel > 3)
  {
    return 0;
  }
  ...
  uint16_t config = ADS1015_REG_CONFIG_CQUE_NONE    | // Disable the comparator (default val)
                    ...
                    ADS1015_REG_CONFIG_CPOL_ACTVLOW | // Alert/Rdy active low   (default val)
                    ADS1015_REG_CONFIG_CMODE_TRAD   | // Traditional comparator (default val)
                    ADS1015_REG_CONFIG_DR_1600SPS   | // 1600SPS  samples per second (default val)
